...
@pseudoElement ${setName}-icons
@categories iconset
${opts.demo ? `@showcase ${setName}-demo.html:${setName}-icons-demo` : ""}
-->
<dom-module id="${setName}-icons">
    <script>Polymer({ is: "${setName}-icons" });</script>
</dom-module>
...
function generateDemo(defs, opts) {
    ...
    const elementPath = path.relative(path.dirname(opts.demoPath), opts.dest).replace(/\\/, "/");
...
<html>
  <head>
    ...
    <link rel="import" href="${elementPath}">
  </head>
  <body>
    <dom-module id="${opts.name}-icons-demo">
      <template>
${
    defs.map(function(def) {
      ...
      <script>
          Polymer({is: "${opts.name}-icons-demo"});
      </script>
    </dom-module>

    <${opts.name}-icons-demo></${opts.name}-icons-demo>
  </body>
</html>
    `;
}
